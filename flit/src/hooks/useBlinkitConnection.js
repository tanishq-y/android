...
export function useBlinkitConnection() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      let response;
      try {
        response = await fetch(apiUrl('/api/v2/connections/blinkit'), {
          method: 'GET',
  ...
  const saveSession = useCallback(async ({ cookieHeader, expiresAt = null }) => {
    ...
    try {
      let saveResponse;
      try {
        saveResponse = await fetch(apiUrl('/api/v2/connections/blinkit/session'), {
          method: 'POST',
          headers: authHeaders,
          body: JSON.stringify({
